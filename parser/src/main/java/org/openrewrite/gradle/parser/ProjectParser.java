...
import org.openrewrite.gradle.toolingapi.parser.JavaSourceSetData;
import org.openrewrite.gradle.toolingapi.parser.KotlinSourceSetData;
...
public class ProjectParser {
    ...
    private void logLifecycle(String s, Object... objects) {
        if (logger instanceof org.gradle.api.logging.Logger) {
            ((org.gradle.api.logging.Logger) logger).lifecycle(s, objects);
        } else {
    ...
    public Stream<SourceFile> parse(GradleProjectData subproject, Set<Path> alreadyParsed, ExecutionContext ctx) {
        ...
        try (ProgressBar progressBar = StringUtils.isBlank(cliPort) ? new NoopProgressBar() :
            ...
            Collection<PathMatcher> exclusions = options.getExclusions().stream()
            ...
            if (isExcluded(exclusions, baseDir.relativize(subproject.getProjectDir().toPath()))) {
                logLifecycle("Skipping project {} because it is excluded", subproject.getPath());
                return Stream.empty();
            }

            logLifecycle("Scanning sources in project {}", subproject.getPath());
            List<NamedStyles> styles = options.getStyles();
            logLifecycle("Using active styles {}", styles.stream().map(NamedStyles::getName).collect(toList()));
            @SuppressWarnings("deprecation")
            List<JavaSourceSetData> sourceSets = subproject.getJavaSourceSets().stream()
            ...
            for (JavaSourceSetData sourceSet : sourceSets) {
                ...
                List<Path> unparsedSources = sourceSet.getSources().stream()
                ...
                if (!javaPaths.isEmpty()) {
                    ...
                    Stream<SourceFile> cus = Stream
                            .of((Supplier<JavaParser>) () -> JavaParser.fromJavaVersion()
                                    ...
                                    .logCompilationWarningsAndErrors(options.getLogCompilationWarningsAndErrors())
                                    .build())
                            .map(Supplier::get)
                ...
                if (subproject.getPlugins().stream().anyMatch(gpd -> gpd.getFullyQualifiedClassName().startsWith(GROOVY_PLUGIN))) {
    ...
    private GradleParser gradleParser() {
        List<Path> settingsClasspath = project.getSettingsClasspath()
                .stream()
                .map(File::toPath)
                .collect(toList());;
    ...
    private SourceFileStream parseGradleFiles(
        ...
        if (project.getBuildscriptFile() != null) {
            ...
            if (!isExcluded(exclusions, buildScriptPath) && buildGradleFile.exists()) {
                ...
                sourceFiles = sourceFiles.map(sourceFile -> sourceFile.withMarkers(sourceFile.getMarkers().add(org.openrewrite.gradle.marker.GradleProject.fromToolingModel(project))));
                    ...
                    sourceFiles = Stream.concat(
                            ...
                            gradleParser
                                    ...
                                    .map(sourceFile -> {
                                        ...
                                        return sourceFile.withMarkers(sourceFile.getMarkers().add(org.openrewrite.gradle.marker.GradleSettings.fromToolingModel(finalGs)));
                                    }));
                    gradleFileCount++;
                }
    ...
    protected SourceFileStream parseNonProjectResources(GradleProjectData subproject, Set<Path> alreadyParsed, ExecutionContext ctx, List<Marker> projectProvenance, Stream<SourceFile> sourceFiles) {
    ...
    private OmniParser omniParser(Set<Path> alreadyParsed) {
        return OmniParser.builder(
                        ...
                        PlainTextParser.builder()
                                .plainTextMasks(baseDir, options.getPlainTextMasks())
                                .build(),
                ...
                .sizeThresholdMb(options.getSizeThresholdMb())
                .build();
    }
    ...
    private SourceFileStream parseMultiplatformKotlinProject(GradleProjectData subproject, Collection<PathMatcher> exclusions, Set<Path> alreadyParsed, ExecutionContext ctx) {
